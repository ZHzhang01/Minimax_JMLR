...
##修订的训练代码
def train_h(network, optimizer, X, W, Z, Y, A,
                       batch, n_epochs, lam_k=1.0, gamma1=1, center_res=True):
    """
...
def train_e(network, optimizer,
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        for i in range(max(1, n // batch)):
            ...
            if center_res:
                phi = phi - phi.mean()
            phi = a_b * 1/(phi.clamp_(0.01, 0.99))
...
momentum = 0.95
ite = 200               # repeats
sample_sizes = [1200]    # can add 1200 later
...
for count, nnn in enumerate(sample_sizes):
    ...
    for iii in range(ite):
        ...
        for i in range(sample_size):
            ...
            u_list[i] = u
            z_list[i] = np.power(TTT @ z,3)
            w_list[i] = np.power(TTT @ w,3)
        ...
        with torch.no_grad():
            ...
            # num  = torch.mean(a_t * y_t / eh)   # E_n[A Y / e]
            # den  = torch.mean(a_t / eh)         # E_n[A / e]
            # J_ipw_hajek = (num / den).item()
...
for count, nnn in enumerate(sample_sizes):
    ...
    for iii in range(ite):
        ...
        for i in range(sample_size):
            ...
            u_list[i] = u
            z_list[i] = np.power(TTT @ z_raw,3)
            w_list[i] = np.power(TTT @ w_raw,3)
